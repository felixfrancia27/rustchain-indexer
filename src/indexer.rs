...
impl BlockIndexer {
    ...
    async fn index_block_internal(
        ...
        let block_opt: Option<Block<H256>> = provider
        ...
        // Get transactions separately - process in parallel if possible
        ...
        let tx_futures: Vec<_> = tx_hashes
            ...
            .map(|(idx, tx_hash)| {
                ...
                let tx_hash = *tx_hash;
                async move {
                    provider.get_transaction(tx_hash).await.map(|tx_opt| {
                        tx_opt.map(|tx| IndexedTransaction {
                            hash: format!("{:?}", tx.hash),
                            from: format!("{:?}", tx.from),
        ...
        Self::convert_block(block, transactions).await
    }

    async fn index_block(&self, block_number: u64) -> Result<()> {
        let block_opt: Option<Block<H256>> = self
            .provider
            .get_block(block_number)
            .await
            .context("Failed to fetch block from RPC")?;
        ...
        let indexed_block = Self::convert_block(block, transactions).await?;
